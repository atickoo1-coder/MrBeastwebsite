...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $result = $productController->update($id, $_POST, $_FILES['image'] ?? [], $_POST['csrf_token'] ?? '');
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <span class="text-secondary" style="font-size:0.85rem;">
            Editing: <?= e(truncateText($product['name'], 50)) ?>
        </span>
    </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="name" name="name"
                               value="<?= e($_POST['name'] ?? $product['name']) ?>" required maxlength="255"
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="5"
                                  placeholder="Enter product description"><?= e($_POST['description'] ?? $product['description']) ?></textarea>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="sku" name="sku"
                               value="<?= e($_POST['sku'] ?? $product['sku']) ?>" required maxlength="100"
                        ...
                        <div class="d-flex gap-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="status"
                                       ...
                                       <?= ((int) ($_POST['status'] ?? $product['status']) === 1) ? 'checked' : '' ?>>
                                <label class="form-check-label" for="statusActive">Active</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="status"
                                       id="statusInactive" value="0"
                                       <?= ((int) ($_POST['status'] ?? $product['status']) === 0) ? 'checked' : '' ?>>
            ...
            <div class="col-12 col-lg-4">
                <div class="card bg-transparent border">
                    <div class="card-body text-center">
                        ...
                        <div id="imagePlaceholder" style="<?= $product['image'] ? 'display:none;' : '' ?> width:200px;height:200px;margin:0 auto;border:2px dashed var(--border-color);border-radius:8px;display:flex;align-items:center;justify-content:center;flex-direction:column;color:var(--text-secondary);">
                            <i class="bi bi-image" style="font-size:2.5rem;margin-bottom:0.5rem;"></i>
                            <span style="font-size:0.85rem;">Click to change image</span>
